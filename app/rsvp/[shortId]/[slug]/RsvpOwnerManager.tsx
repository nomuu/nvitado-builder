...
import { UserCheck, UserX, Trash2, Loader2, Link2, Mail, Phone, RefreshCw, Undo2 } from 'lucide-react';
          ...
          {guests.map((guest) => (
            <div key={guest.id} className="p-3 bg-slate-50 rounded-xl border border-slate-100 space-y-2">
              ...
              <div className="flex items-center gap-2 pt-1">
                {/* ACCEPT — hidden when already accepted */}
                {guest.status !== 'going' && (
                  <button
                ...
                {/* RESET TO PENDING — only when a decision was made */}
                {guest.status && (
                  <button
